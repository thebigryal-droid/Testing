...
window.closeModalSafe = () => {
    ...
    window.sendWaReminder = () => { 
        ...
        let sessionsDone = Object.keys(c.logs || {}).length;
        let currentWt = (c.bca && c.bca.length) ? c.bca[0].w + "kg" : "Not Logged";
        
        // Build the automated message
        let msg = `*RYAL FITHUB PRO UPDATE* 🏆\n\nHey ${c.name.split(' ')[0]},\nHere is your current progress summary:\n\n`;
        msg += `📈 *Sessions Completed:* ${sessionsDone} / ${c.sess || '?'}\n`;
        msg += `⚖️ *Current Weight:* ${currentWt}\n`;
        
        if (c.macros) {
            msg += `🍗 *Daily Macros:* ${c.macros.p}g P | ${c.macros.c}g C | ${c.macros.f}g F\n`;
        }
        
        if (c.slot) {
            msg += `\n⏰ *Next Session:* ${window.formatTime(c.slot)}\n`;
        }
        
        msg += `\nKeep up the great work! 💪\n- Coach ${window.trainerProfile.name.split(' ')[0]}`; 
        ...
        if(dates.length === 0) { html = '<div style="text-align:center; color:var(--text-muted); font-size:12px; padding:10px;">No workouts logged yet.</div>'; } 
            ...
            dates.forEach(d => { 
                let logDetails = c.logs[d].map(ex => { 
                    ...
                    let tagHtml = ex.tag && ex.tag !== 'Normal' ? `<span class="badge" style="background:var(--primary); color:white; font-size:9px;">${ex.tag.toUpperCase()}</span>` : '';
                    return `<div style="padding:12px 0; border-bottom:1px solid rgba(0,0,0,0.05);"><div style="font-weight:800; font-size:13px; margin-bottom:8px; color:var(--text); display:flex; align-items:center; gap:8px;">${ex.name} ${tagHtml}</div><div style="display:flex; flex-wrap:wrap; gap:6px;">${setsHtml}</div></div>`; 
                }).join(''); 
    ...
    window.calc1RM = (i) => { 
        let w = parseFloat(document.getElementById(`inW_${i}`).value) || 0; 
        let r = parseInt(document.getElementById(`inR_${i}`).value) || 0; 
        let rmEl = document.getElementById(`rm_${i}`); 
        if (w > 0 && r > 0) { 
            let max = Math.round(w * (36 / (37 - r))); // Clinical Brzycki Formula
            rmEl.innerText = max + "kg Est. 1RM"; 
            rmEl.style.color = "var(--primary)"; 
        } else { 
            rmEl.innerText = "0kg Est. 1RM"; 
            rmEl.style.color = "var(--text-muted)"; 
        } 
    };
    ...
    window.generateSetInputs = () => { 
        ...
        for (let i = 1; i <= c; i++) { 
            h += `<div style="display:flex; flex-wrap:wrap; gap:10px; margin-bottom:12px; align-items:center; background:var(--bg); padding:12px; border-radius:12px; border:1px solid var(--border);">
                <div style="display:flex; gap:10px; width:100%;">
                    <span style="font-size:12px; font-weight:800; color:var(--text-muted); width:30px; display:flex; align-items:center;">S${i}</span>
                    <input type="number" id="inW_${i}" placeholder="Weight (kg)" style="margin:0; background:var(--card); flex:1;" oninput="calc1RM(${i})">
                    <input type="number" id="inR_${i}" placeholder="Reps" style="margin:0; background:var(--card); flex:1;" oninput="calc1RM(${i})">
                </div>
                <div style="width:100%; text-align:right; font-size:11px; font-weight:800; text-transform:uppercase;">
                    <i class="fas fa-bolt" style="width:10px; color:var(--warning);"></i> <span id="rm_${i}" style="color:var(--text-muted);">0kg Est. 1RM</span>
                </div>
            </div>`; 
        } 
    ...
    window.saveExercise = async () => { 
        ...
        const tag = document.getElementById('exIntensityTag').value; // Captures the new intensity tag
        ...
        // Injects the tag into the clinical database payload
        cl.logs[d].push({ id: Date.now(), workoutName: wName, name: window.curExName, sets: sets, tag: tag }); 
    ...
    // Core AI Fetch wrapper to prevent hanging the web app
        ...
        const actualTimeout = 25000; // Force a generous 25-second limit for mobile networks
        ...
        try {
            ...
            if (error.name === 'AbortError' || error.message.includes('aborted')) {
                throw new Error("Connection timed out. The network is slow or the AI took too long (25s limit).");
            }
            throw error;
        }
    };
    ...
    window.loginVIP = async () => { 
        ...
        skelDiv.innerHTML = `
            ...
            <div style="display:flex; gap:12px; margin-bottom:25px;">
                <div class="skeleton-box skel-card" style="flex:1; height:60px;"></div>
                <div class="skeleton-box skel-card" style="flex:1; height:60px;"></div>
                <div class="skeleton-box skel-card" style="flex:1; height:60px;"></div>
            </div>
    ...
    window.renderLiveVIP = () => { 
        ...
        document.getElementById('vipStatFat').innerText = (c.bca && c.bca.length) ? `${c.bca[0].pbf}%` : "-";
                // Render VIP Macro Progress Bars
        let macroContainer = document.getElementById('vipMacroBarsContainer');
        if (c.macros && c.tdee) {
            let pPct = Math.min(100, (c.macros.p * 4 / c.tdee) * 100);
            let cPct = Math.min(100, (c.macros.c * 4 / c.tdee) * 100);
            let fPct = Math.min(100, (c.macros.f * 9 / c.tdee) * 100);
            
            macroContainer.innerHTML = `
            <div class="section" style="border-top:4px solid #10b981;">
                <h3 style="font-size:12px;margin:0 0 15px 0;text-transform:uppercase;color:var(--text);"><i class="fas fa-utensils" style="color:#10b981;margin-right:8px;"></i> Daily Nutrition Targets</h3>
                
                <div style="margin-bottom:12px;">
                    <div style="display:flex; justify-content:space-between; font-size:11px; font-weight:800; color:var(--text); margin-bottom:4px;">
                        <span>PROTEIN (${c.macros.p}g)</span><span>${Math.round(pPct)}% of Diet</span>
                    </div>
                    <div style="background:var(--border); height:8px; border-radius:4px; overflow:hidden;">
                        <div style="background:#ef4444; width:${pPct}%; height:100%; border-radius:4px;"></div>
                    </div>
                </div>
                
                <div style="margin-bottom:12px;">
                    <div style="display:flex; justify-content:space-between; font-size:11px; font-weight:800; color:var(--text); margin-bottom:4px;">
                        <span>CARBS (${c.macros.c}g)</span><span>${Math.round(cPct)}% of Diet</span>
                    </div>
                    <div style="background:var(--border); height:8px; border-radius:4px; overflow:hidden;">
                        <div style="background:#3b82f6; width:${cPct}%; height:100%; border-radius:4px;"></div>
                    </div>
                </div>
                
                <div style="margin-bottom:12px;">
                    <div style="display:flex; justify-content:space-between; font-size:11px; font-weight:800; color:var(--text); margin-bottom:4px;">
                        <span>FATS (${c.macros.f}g)</span><span>${Math.round(fPct)}% of Diet</span>
                    </div>
                    <div style="background:var(--border); height:8px; border-radius:4px; overflow:hidden;">
                        <div style="background:#f59e0b; width:${fPct}%; height:100%; border-radius:4px;"></div>
                    </div>
                </div>
                
                <div style="text-align:center; font-size:13px; font-weight:800; color:var(--text-muted); margin-top:15px; background:var(--bg); padding:8px; border-radius:8px; border:1px solid var(--border);">
                    TARGET TDEE: <span style="color:#10b981;">${c.tdee} KCAL</span>
                </div>
            </div>`;
        } else {
            macroContainer.innerHTML = '';
...
window.addEventListener('beforeinstallprompt', (e) => {
    ...
    // Show the install button if the user is on the VIP portal
    const installBtn = document.getElementById('vipInstallBtn');
    if (installBtn) installBtn.style.display = 'flex';
});

window.installVipApp = async () => {
    if (window.deferredPrompt) {
        ...
        if (outcome === 'accepted') {
            document.getElementById('vipInstallBtn').style.display = 'none';
        }
